...
readonly FIXTURE_DIR="${REPO_ROOT}/proto/fixtures/runtime-v1.5"

if (( $# > 1 )) || [[ "${MODE}" != "check" && "${MODE}" != "generate" ]]; then
    echo "usage: scripts/protocol15-fixtures.sh [check|generate]" >&2
    exit 64
fi

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/diskplan-protocol15-fixtures.XXXXXX")"
readonly TMP_ROOT
...
if [[ "${MODE}" == "generate" ]]; then
    ...
    echo "generated protocol 1.5 runtime fixtures"
else
    cargo run --locked --quiet -p generated-source-publish -- \
        verify \
        "${REPO_ROOT}" \
        ...
        "${CODEX_SOURCE}" "${CODEX_DESTINATION}" \
        "${VERSION_SOURCE}" "${VERSION_DESTINATION}"
    echo "protocol 1.5 runtime fixtures match the Swift authority"
fi

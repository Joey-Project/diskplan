...
readonly FIXTURE_DIR="${REPO_ROOT}/proto/fixtures/runtime-v1.6"

if (( $# > 1 )) || [[ "${MODE}" != "check" && "${MODE}" != "generate" ]]; then
    echo "usage: scripts/protocol16-fixtures.sh [check|generate]" >&2
    exit 64
fi

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/diskplan-protocol16-fixtures.XXXXXX")"
readonly TMP_ROOT
...
readonly FAILURE_SOURCE="${TMP_ROOT}/execution-stream-failure.frames.hex"
readonly FAILURE_DESTINATION="${FIXTURE_DIR}/execution-stream-failure.frames.hex"
...
if [[ "${MODE}" == "generate" ]]; then
    cargo run --locked --quiet -p generated-source-publish -- \
        ...
        "${FAILURE_SOURCE}" "${FAILURE_DESTINATION}"
    echo "generated protocol 1.6 runtime fixtures"
else
    cargo run --locked --quiet -p generated-source-publish -- \
        verify \
        "${REPO_ROOT}" \
        ...
        "${VERSION_SOURCE}" "${VERSION_DESTINATION}" \
        "${FAILURE_SOURCE}" "${FAILURE_DESTINATION}"
    echo "protocol 1.6 runtime fixtures match the Swift authority"
fi

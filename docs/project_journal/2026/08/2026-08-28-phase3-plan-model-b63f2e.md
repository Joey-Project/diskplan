...
- Visible-row materialization is bounded to the viewport, while structural validation rejects invalid or duplicate action, target, waiver, blocker, prerequisite, and release-set identities.
...
- Fourteen focused model tests cover hierarchy order, invalid IDs, release-set consistency, cursor/filter/resize behavior, group-local stable sorting, target-path isolation, invalidation, and a 20,000-action virtualized projection.
...
- `INSTA_UPDATE=no cargo test --locked --workspace` passed 40 library tests, 9 fake-engine process tests with 4 explicit real-engine ignores, 8 core tests, 5 canonical tests, and 20 generated-source publisher tests.

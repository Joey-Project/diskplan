...
- The scanner-integrated probe branch passed 32 focused `DiskplanMacOSTests` and all 43 tests in its then-current Swift package after the packed-layout fix.
- The fixture-hardening branch passed 30 focused `DiskplanMacOSTests` and all 133 tests in its then-current Swift package with serial execution.
- The combined Phase 0 integration merge passed 33 focused `DiskplanMacOSTests` and the
  controlled `diskplan-macos-probe --self-test`; both India-only hooks remained explicitly
  `not-available` without their opt-in environment variables.
...
- Both source branches passed their then-current complete local `scripts/test-macos-capabilities.sh` gate and controlled CLI probe; each left the extension-backed and real APFS volume-group India-host oracles as explicit `not-available` hooks without claiming either live gate passed.

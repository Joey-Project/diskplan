...
- Phase: local fixture implementation and compile-only validation.
- Next step: independent review, then copy or check out the reviewed head on the India host and
  run the signed lifecycle.
...
- `scripts/test-macos-capabilities.sh`: 17 existing `DiskplanMacOS` tests and the live self-test
  passed; the signed fixture remained explicitly unavailable without its opt-in environment
  variable.

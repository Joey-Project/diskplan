...
- `cargo test --locked --workspace` (2 cleanup unit tests, 8 process tests, 2 explicit cross-language ignores, 8 core tests, 5 canonical tests, and 20 publisher tests).
...
- Full-range review through `2b0ccdd1ef3e47f12d4ea2d328d4fd74b4f19e66` found one remaining gap: group-writable intermediate ancestors were not part of the trusted namespace proof. The subsequent fix traverses and seals every destination ancestor from a descriptor-held repository root and detects detached or replaced ancestor chains.

...
same held descriptor. Provider ownership beneath `/` is still resolved from item
evidence and the authoritative boundary probe; the exception does not make
descendants local by path convention.
